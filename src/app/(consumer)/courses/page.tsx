  ...
  return courses.map((c) => (
    <Card key={c.id} className="overflow-hidden flex flex-col">
      <CardHeader>
        ...
        <CardDescription>
          {formatPlural(c.sectionsCount, {
            singular: "section",
            plural: "sections",
          })}{" "}
          •{" "}
          {formatPlural(c.lessonsCount, {
            singular: "lesson",
            plural: "lessons",
          })}
        </CardDescription>
      </CardHeader>
      <CardContent className="line-clamp-3" title={c.description}>
        {c.description}
      </CardContent>
      ...
      <div
        ...
        style={{
          width: `${(c.lessonsComplete / c.lessonsCount) * 100}%`,
        }}
      />
    </Card>
  ));
...
async function getUserCourses(userId: string) {
  ...
  const courses = await db
    .select({
      ...
      lessonsComplete: countDistinct(UserLessonComplete.lessonId),
    })
    .from(CourseTable)
    .leftJoin(
      UserCourseAccessTable,
    ...
    .leftJoin(
      UserLessonComplete,
      and(
        eq(UserLessonComplete.lessonId, LessonTable.id),
  ...
  courses.forEach((course) => {
    cacheTag(
      ...
      getCourseSectionIdTag(course.id),
